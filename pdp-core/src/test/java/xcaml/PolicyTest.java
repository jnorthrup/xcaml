...
public class PolicyTest extends CamelTestSupport {
  ...
  public void testPolicy1() throws Exception {
    ...
    PolicyType policyType = mockPolicySource.assertExchangeReceived(0).getIn().getBody(PolicyType.class);
    ...
    RequestType requestType = mockRequestSource.assertExchangeReceived(0).getIn().getBody(RequestType.class);
    class Evaluator {

      XacmlCombiningAlgorithm combiningAlgorithm = XacmlCombiningAlgorithm.from(policyType.getRuleCombiningAlgId());
  ...
  protected RouteBuilder createRouteBuilder() throws Exception {
    return new RouteBuilder() {
      ...
      public void configure() throws Exception {
        ...
        from(POLICY_UNMARSHALL)
            .unmarshal(jaxbDataFormat)
            .to(mockPolicySource);

        from(REQUEST_UNMARSHALL)
            .unmarshal(jaxbDataFormat)
            .to(mockRequestSource);
